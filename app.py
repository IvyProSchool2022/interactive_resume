...
# Function to query GPT using LangChain
def query_gpt(user_input):
    # Initialize LangChain ChatOpenAI (for chat-based models)
    ...
    # Generate the chat prompt with user input
    prompt = chat_prompt.format_messages(input_text=user_input)
                ...
                for i, work in enumerate(st.session_state.work_experiences):
                    with st.expander(f"{work['company_name']} - {work['location']}"):
                        ...
                        if st.button(f"Remove Work Experience {i + 1}", key=f"remove_work_{i}"):
                            workexp_to_remove = i
                            st.success(f"Marked Work Experiences '{work['company_name']}' for removal.")
                            break
                if workexp_to_remove is not None:
                    st.session_state.projects.pop(workexp_to_remove)
